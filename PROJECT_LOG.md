...
### Feature — Inline buttons Phase B (stash architecture) — PR #69 merged, issue #66 closed

- **PR #69** merged — `feat(n8n): inline buttons Phase B — stash architecture for callback resolution` (squash merge commit `290b5cb`). Replaces the Phase A `callback_data` shape from PR #67 (`j:<slug>` / `s:<slug>`) with `j:<shortid>` / `s:<shortid>`, where `<shortid>` is a deterministic 8-char hex hash that indexes into a new `state/pending-callbacks.json` stash. The stash is **written** when an email triggers a Telegram notification (Kaggle Email Watcher's new `Stash Pending Callback` Code node) and **read** when the user clicks a button (Telegram Callback Handler's refactored `Parse Callback Data` node). The handler exposes the resolved fields (`competition_id`, `competition_url`, `competition_name`, `email_id`, `chat_id`, `message_id`) downstream so #46 (skip) and #28 (accept) consumers get a complete context object on click — without re-deriving anything from the URL or the slug, and without the 64-byte `callback_data` cap that Phase A bumped against on most real Kaggle slugs.
- **Why Phase B over Phase A** — two reasons:
  - **64-byte limit**: Phase A `j:<slug>` worked for ~95% of real Kaggle slugs (`titanic` is fine at 9 bytes, but `house-prices-advanced-regression-techniques` reaches 45 and `llm-classification-finetuning-2026-spring-hackathon-edition` blows the cap at 61). Phase B's 11-byte constant fits any slug.
  - **Downstream context**: #46 needs the Gmail `email_id` to mark-as-read + apply the `KAGGLE/Declined` label; #28 needs the canonical `competition_url` for repo bootstrap. The stash is the contract that decouples the click-time handler from the email-time parser — without it, each consumer would have to re-query Gmail or re-parse the original message (fragile coupling).
- **Project scaffolding** introduced or extended:
  - `rules/pending-callbacks.schema.json` (NEW) — JSON Schema Draft-07 mirroring `rules/watchlist.schema.json` (PR #54). Required: `shortid`, `competition_id`, `competition_url`, `competition_name`, `chat_id`, `email_id`, `ts`. `additionalProperties: false`.
  - `state/pending-callbacks.json.example` (NEW) — empty array `[]` bootstrap (runtime file gitignored).
  - `.gitignore` — adds `state/pending-callbacks.json` next to `state/watchlist.json`.
  - `scripts/validate-rules.sh` — extended with the same `Draft7Validator` + custom `FormatChecker` (`date-time`, `uri`) block as the watchlist validator, **plus** an explicit shortid-uniqueness check after the schema (catches any future writer regression or manual edit that introduces a duplicate; the handler resolves callbacks via `list.find((e) => e.shortid === suffix)` so a duplicate would silently route to whichever entry came first).
  - `docs/pending-callbacks.md` (NEW) — operator + developer documentation, mirrors `docs/watchlist.md`. Writer + reader Code-node templates, bootstrap procedure, 30-day pruning rule, position-after-Inject-Chat-ID rationale (so the chat_id is the configured `default_chat_id` not the empty per-rule placeholder), and concurrency note attributed to the writer side.
  - `docker/docker-compose.yml` — `NODE_FUNCTION_ALLOW_BUILTIN=fs` → `fs,crypto` (needed for `crypto.createHash('sha256')` in the writer; `fs` was already enabled in PR #53). Container force-recreated to apply the env change.
- **Workflow changes** (re-exported from the n8n UI, sanitised via the canonical jq filter from `CONTRIBUTING.md`):
  - `workflows/kaggle-email-watcher.json` — re-published as **v2.1.0** (`Move Stash node after Inject Chat ID + empty-slug guard`). Three internal modifications across the PR's lifetime:
    - `Parse Email` Code node now also captures `email_id: $input.first().json.id` from the Gmail Trigger output, so the stash writer downstream has a Gmail message id to persist.
    - New `Stash Pending Callback` Code node inserted **after `Inject Chat ID`** (and before `Route by Action Type`) — computes `shortid = sha256(competition_id + ts).slice(0, 8)`, prunes entries older than 30 days, appends a fresh entry, exposes `shortid` on its output JSON for the downstream Send Telegram inline-keyboard expressions. Throws on empty `competition_id` so a malformed Kaggle URL upstream surfaces via the global Error Handler instead of writing a useless entry. The position-after-Inject-Chat-ID part is the Copilot Must-fix described below; the original `after Match Rule` placement was caught before the workflow ever served a real callback.
    - `Send Telegram` inline-keyboard buttons now emit `=j:{{ $json.shortid }}` and `=s:{{ $json.shortid }}` instead of the Phase A `=j:{{ $json.slug }}` / `=s:{{ $json.slug }}`. Single `=` expression prefix preserved cleanly (verified via JSON re-export).
  - `workflows/telegram-callback-handler.json` — re-published as **v2.2.0** (`Surface stash failures + route hex-miss to unknown`). Single Code node refactored across three commits:
    - `Parse Callback Data` now detects three callback_data shapes in priority order: (1) legacy URL-encoded `action=<verb>&competition_id=<slug>` when `=` is in the rawData, (2) compact `<j|s>:<shortid|slug>` when `:` is at position 1 with stash lookup on the suffix, (3) Phase A slug fallback (inside path 2) when stash misses and the suffix is not a hex hash.
    - On stash hit: returns the action mapped from prefix plus all the resolved fields from the stash entry.
    - On stash read error other than ENOENT: throws (rather than silently swallowing) so a corrupt or unreadable stash file surfaces via the global Error Handler with the underlying filesystem error in the PR #61 Detail line.
    - On hex-shaped suffix with stash miss (entry pruned >30d, never written, file unreadable): resets `action` to `unknown` so the Switch fallback handles it loudly via Answer Unknown rather than routing the click to join/skip with all resolved fields empty.
    - Live `message_id` from `callback_query.message.message_id` is preferred over any stashed value (more authoritative).

- **Validation evidence** — 4 simulated curl paths exercised end-to-end with seeded test entry, outputs inspected via the n8n SQLite `execution_data` table after WAL checkpoint:

  | Path | Input | action | competition_id | stash_hit |
  |---|---|---|---|---|
  | 1 (URL-encoded — Codex P2 regression target) | `action=skip&competition_id=titanic` | `skip` ✓ | `titanic` ✓ | `false` |
  | 2 (compact stash hit, seeded `abc12345`) | `s:abc12345` | `skip` ✓ | `titanic` + `competition_url` + `competition_name` + `email_id` ✓ | `true` ✓ |
  | 3 (compact slug fallback — Phase A backward-compat) | `s:titanic` | `skip` ✓ | `titanic` ✓ | `false` |
  | 4 (hex hash miss — Copilot safety) | `s:deadbeef` | `unknown` ✓ (post-fix; was `skip` with empty fields before) | `''` (safe) | `false` |

  Path 2 is the headline Phase B path; it proves #46 / #28 will have everything they need to act on the email and journal the decision without further infrastructure work. Path 4 is the Copilot fix that distinguishes "stash entry pruned" from "click is malformed" — both now route to `unknown`, surfacing via Answer Unknown.

- **Decisions log**:
  - `Picked Option B (Phase B before #46) over Options A and C` — when planning the Skip branch (#46), surfaced that 2 of its 3 side-effects (Gmail mark-as-read + watchlist write with `source_email_id`) require fields the Phase A `callback_data` does not carry. Considered: A) ship #46 partial with watchlist schema hack and deferred mark-as-read (~1h, but introduces tech debt to rework when #66 ships); B) ship #66 first then #46 cleanly (~3h up-front but architecturally clean); C) Gmail email-search workaround at click time (fragile, slow, off-pattern). Picked B for the architectural cleanliness; the actual ~3h estimate held in practice. Documented as a project-level lesson because it is a recurring pattern: when a downstream issue's contract requires a richer upstream artifact, building the upstream first and the downstream second avoids a forward-incompatible commit.
  - `Promoted #66 from milestone v1.3 to v1.2` — the original triage placed Phase B under v1.3 (Analyse IA) because it was a follow-up to #56 / #67. With Option B picked, #66 became a prerequisite for the #46 / #28 work that lives in v2.0, which itself blocks the v1.3 epic. Promoting to v1.2 (Telegram interactif — ngrok) reflects the real shipping order and keeps the milestone semantics honest.

- **Reviews** — both bots posted within minutes of the PR opening:
  - **Codex** posted 1 P2 (URL-encoded format dropped). Fixed in commit `068243d` by restoring the legacy URL-encoded parser as detection priority 1 (when `=` in rawData), with a 4-path validation table on the inline reply.
  - **Copilot** posted 8 inline comments. 1 was a doublon of the Codex P2 (cross-referenced in the reply). The remaining 7 split into 1 Must (chat_id placement: stash node was running before Inject Chat ID so the stashed chat_id was always the empty per-rule placeholder) and 6 Should (silent stash read errors; empty-slug guard; shortid uniqueness check; doc curl shape inconsistency; chat_id template doublon; concurrency note misattribution). All 7 distinct points addressed in commit `6139ec1` — the Must by re-routing the workflow connections from `Match Rule → Stash → Read Config` to `Match Rule → Read Config → Inject Chat ID → Stash → Route by Action Type`, the Shoulds by code amendments, validate-rules extension, and doc precision fixes. All 8 comments replied to inline with SHA references; threads left unresolved per the repo convention (operator resolves manually in GitHub UI).

- **Cumulative tally for the day** — 1 PR merged (#69, the only feature shipping today), 1 issue closed (#66, by `Closes #66` in the PR body). With this PR, **the Phase B contract is in place and #46 + #28 are unblocked from the field-availability standpoint**: the resolved `email_id`, `competition_url`, `competition_name` are all surfaced by `Parse Callback Data` whenever the click came through the stash path. The next concrete features (#46 skip — mark email read + edit Telegram message + watchlist write; #28 accept — repo bootstrap from competition_url) can each be one focused workflow change without further infrastructure work.

## 2026-06-17

### Incident — Telegram notifications silently stopped (regression from PR #69 empty-slug throw)

- Symptom: operator received a new Kaggle Competition Launch email but **no Telegram notification** fired. Workflow had been running fine before; this is a regression.
- Root cause: the `Stash Pending Callback` Code node added in **PR #69** (Phase B, 2026-05-05) **throws** when the competition slug is empty:
  - `if (!competitionId) { throw new Error('... refusing to write entry without competition slug ...'); }`
  - Because `Stash Pending Callback` sits **before** `Send Telegram` in the chain (`... → Inject Chat ID → Stash Pending Callback → Route → Send Telegram`), the throw aborts the **entire** execution before the notification is ever sent. Any single email whose Kaggle URL cannot be extracted now suppresses the alert completely.
  - Slug emptiness is realistic, not theoretical: Kaggle launch emails frequently wrap the destination in a **click-tracking redirect** (SendGrid `ct.sendgrid.net`, `email.kaggle.com`, …). The `www.kaggle.com/competitions/<slug>` link then appears only **percent-encoded inside a `?url=...` parameter**, which `Parse Email`'s direct-link-only regex did not match → `url` fell back to the bare `/competitions` default → empty slug → throw.
  - Net effect: PR #69 converted a previously-tolerated degraded case (empty slug → `j:`/`s:` buttons mapped to `unknown` by the handler, **notification still sent**) into a hard failure that drops the notification entirely — strictly worse for the user.
- Fix (this session, on branch `claude/workflow-notification-debug-z9b4cq`):
  - `Parse Email` — URL extraction now searches the raw `html`/`body` first, then a **percent-decoded copy** of each (decoded per `%XX` token so a stray `%` never throws), so a Kaggle link wrapped inside a redirect's `?url=` param is recovered. Verified: a SendGrid-wrapped `...%2Fcompetitions%2Fneurips-2026-grand-challenge%2Foverview...` link now resolves to slug `neurips-2026-grand-challenge`; direct links still work unchanged.
  - `Stash Pending Callback` — replaced the `throw` with **graceful degradation**: an empty slug falls back to `competition_id = 'unknown'` (which the Telegram Callback Handler already maps via its Switch fallback) so the **notification always fires**. The stash entry still carries `competition_url` / `competition_name` / `email_id`, so a button click keeps real context. Output now exposes `slug_resolved` (boolean) for observability. `shortid` hash seed extended with `email_id` so distinct `unknown` emails in the same millisecond still get distinct ids.
- **Convention note** — per `CLAUDE.md`, `workflows/*.json` is normally edited in the n8n UI and re-exported, never by hand. This change was applied directly to the JSON at the operator's explicit request (no PC access at the time of the incident). **Action required**: re-import `workflows/kaggle-email-watcher.json` into the live n8n instance and re-publish to actually restore service — the committed JSON alone does not update the running workflow.
- Verification done remotely: `make validate` passes (all 5 workflows valid JSON); Node simulation confirms both the redirect-unwrap and direct-link paths. Not yet verified against the live n8n executions (operator to confirm the failing execution shows the `Stash Pending Callback` throw, and that the re-imported workflow sends the alert).
